...
\begin{itemize}
    ...
    \item \textbf{Weighting Hyperparameters:} $x_{max} = 100$, $\alpha = 1.0$.
...
\begin{table}[H]
    ...
    \begin{tabular}{lccc}
        ...
        GLOVE-MLP & 300 & \textbf{0.9072} & \textbf{0.5648} \\
        \midrule
...
% Use resizebox to force the table to fit the page width
\begin{table}[H]
    \centering
    \caption{Nearest Neighbors Comparison: Raw vs. TF-IDF SVD}
    \resizebox{\textwidth}{!}{%
    \begin{tabular}{lll}
        \toprule
...
\noindent \textbf{Observation:} TF-IDF forced the SVD to capture deeper semantic relevance. For example, "India" shifted from generic business nouns (\texttt{engineer}, \texttt{model}) to highly specific historical/regional concepts (\texttt{Indian}, \texttt{partition}).

\subsection*{5.2 Quality Check 2: MLP Evaluation}
Training the MLP on the TF-IDF SVD vectors yielded:
\begin{itemize}
    \item \textbf{Raw SVD (d=300) Baseline:} Acc: 0.8617 $|$ F1: 0.3333
    \item \textbf{TF-IDF SVD (d=300) Score:} Acc: 0.8581 $|$ F1: 0.3092
\end{itemize}
...
\newpage
\subsection{Additional Interesting Visualizations}

\begin{figure}[H]
    \centering
    % Placeholder: Replace with your actual image path
    \includegraphics[width=1\textwidth]{tsne300.png}
    \caption{t-SNE of 300 Random Words colored by Frequency}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{t-SNE Word Clusters for GloVe vs SVD}
\end{figure}
...
\begin{figure}[H]
    \centering
    % Placeholder: Replace with your actual image path
    \includegraphics[width=1\textwidth]{ner.png}
    \caption{NER Performance Macro-F1}
\end{figure}

\begin{figure}[H]
    \centering
    % Placeholder: Replace with your actual image path
    \includegraphics[width=1\textwidth]{macrof1.png}
    \caption{Macro-F1 and Accuracy vs Embedding Dimension}
\end{figure}
